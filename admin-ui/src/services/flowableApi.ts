...
export const processApi = {
  ...
  // Deploy process
  deployProcess: async (file: File, deploymentName: string, category?: string): Promise<any> => {
    const formData = new FormData();
    formData.append('file', file);
    formData.append('deploymentName', deploymentName);
    if (category) formData.append('category', category);

    const response = await api.post('/flow/processes/deploy', formData, {
  ...
  getProcesses: async (): Promise<any[]> => {
    const response = await api.get('/flow/processes/definitions');
    return response.data as any[];
  },

  // Get process BPMN XML
  getProcessBpmn: async (processDefinitionId: string): Promise<string> => {
    const response = await api.get(`/process-definitions/${processDefinitionId}/xml`);
    return response.data as string;
  },

  // Monitoring endpoints
  getQueueStats: async (): Promise<any[]> => {
    const response = await api.get('/monitor/queues');
    return response.data as any[];
  },

  getRabbitMQHealth: async (): Promise<string> => {
    const response = await api.get('/monitor/rabbitmq/health');
    return response.data as string;
  },

  // Version management
  getProcessVersions: async (processKey: string): Promise<any[]> => {
    const response = await api.get(`/process-definitions?key=${processKey}`);
    return response.data as any[];
  },

  compareVersions: async (version1Id: string, version2Id: string): Promise<any> => {
    const response = await api.get(`/process-definitions/compare?v1=${version1Id}&v2=${version2Id}`);
    return response.data as any;
  },

  rollbackToVersion: async (processKey: string, version: number): Promise<any> => {
    const response = await api.post(`/process-definitions/${processKey}/rollback`, { version });
    return response.data as any;
  },

  getProcessMetadata: async (processDefinitionId: string): Promise<any> => {
    const response = await api.get(`/process-definitions/${processDefinitionId}/metadata`);
    return response.data as string;
  },
